    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.06)]">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-heading font-semibold text-cloud-white">PAI Brain</h2>
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-[rgba(139,92,246,0.1)] text-neon-purple">
            Algorithm v1.8.0
          </span>
        </div>
        ...
        {tabs.map(t => {
          ...
          return (
            ...
            >
              ...
              <span>{t.label}</span>
              {t.count && (
                <span className="text-[9px] px-1 py-0.5 rounded bg-[rgba(255,255,255,0.06)]">{t.count}</span>
              )}
            </button>
          )
        })}
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        {tab === 'algorithm' && (
          <>
            ...
            <div className="glass-card p-4">
              <h3 className="text-sm font-heading font-semibold text-cloud-white mb-4">7-Phase Algorithm Flow</h3>
                  ...
                  return (
                    <div key={phase.id} className="flex items-center gap-3">
                      <div className="flex items-center gap-2 w-8 justify-center">
                        <span className="text-[10px] text-steel-gray font-mono">{i + 1}</span>
                      </div>
                      <div
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-heading font-semibold text-cloud-white">{phase.name}</span>
                            <span className="text-[10px]">{phase.emoji}</span>
                          </div>
                          <p className="text-[10px] text-steel-gray truncate">{phase.description}</p>
                        </div>
                      </div>
                      {i < ALGORITHM_PHASES.length - 1 && (
                        <ChevronRight className="w-3 h-3 text-steel-gray/30 shrink-0 rotate-90" />
                      )}
                    </div>
                  )
                })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-xs font-heading font-semibold text-cloud-white">Ideal State Criteria</h4>
                </div>
                <p className="text-[10px] text-steel-gray leading-relaxed">
              ...
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-xs font-heading font-semibold text-cloud-white">Memory v2</h4>
                </div>
                <p className="text-[10px] text-steel-gray leading-relaxed">
              ...
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-xs font-heading font-semibold text-cloud-white">Security Layer</h4>
                </div>
                <p className="text-[10px] text-steel-gray leading-relaxed">
            ...
            {teams.map(team => (
              <div key={team} className="glass-card p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />
                  <h3 className="text-sm font-heading font-semibold text-cloud-white">{team}</h3>
                  ...
                  {AGENTS.filter(a => a.team === team).map(agent => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-[rgba(255,255,255,0.06)] text-steel-gray ml-auto">
                          {agent.role}
                        </span>
                      </div>
                      {selectedAgent === agent.id && (
                        <p className="text-[10px] text-steel-gray mt-1 animate-in fade-in duration-200">
                          {agent.description}
                        </p>
                      )}
                    </button>
        ...
        {tab === 'skills' && (
          <>
            <div className="flex items-center gap-4 text-[11px] text-steel-gray mb-1">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {categories.map(cat => (
                <div key={cat} className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-steel-gray uppercase">{cat}</span>
                </div>
              ))}
            </div>
              ...
              {SKILLS.map(skill => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    />
                    <span className="text-xs font-heading font-semibold text-cloud-white">{skill.name}</span>
                    <code className="text-[9px] text-steel-gray ml-auto font-mono">{skill.prefix}</code>
                  </div>
                  <div className="flex items-center gap-3 text-[10px] text-steel-gray">
                    <span>{skill.commands} cmd</span>
                    <span>{skill.wizards} wiz</span>
                    <span className="uppercase text-[9px] px-1 py-0.5 rounded"
                          style={{ backgroundColor: `${CATEGORY_COLORS[skill.category]}15`, color: CATEGORY_COLORS[skill.category] }}>
                      {skill.category}
                    </span>
                  </div>
                  {selectedSkill === skill.id && (
                    <p className="text-[10px] text-steel-gray mt-2 animate-in fade-in duration-200">
                      {skill.description}
                    </p>
                  )}
                </button>
        ...
        {tab === 'hooks' && (
          <div className="glass-card p-4">
            <div className="flex items-center gap-2 mb-4">
              <Shield className="w-4 h-4 text-coral-energy" />
              <h3 className="text-sm font-heading font-semibold text-cloud-white">Hook Pipeline</h3>
              <span className="text-[10px] text-steel-gray ml-auto">{HOOKS.length} hooks active</span>
            </div>
            <div className="space-y-2">
              {HOOKS.map((hook, i) => (
                <div key={hook.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-[rgba(255,255,255,0.02)]">
                  <span className="text-[10px] text-steel-gray font-mono w-5 text-right shrink-0">{i + 1}</span>
                  <div className="w-1.5 h-1.5 rounded-full bg-success-green shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-cloud-white font-medium">{hook.name}</span>
                      <code className="text-[9px] px-1.5 py-0.5 rounded bg-[rgba(255,107,107,0.1)] text-coral-energy font-mono">
                        {hook.event}
                      </code>
                    </div>
                    <p className="text-[10px] text-steel-gray truncate">{hook.description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-[rgba(255,255,255,0.06)]">
              <h4 className="text-[10px] text-steel-gray uppercase tracking-wide mb-2">Event Flow</h4>
              <div className="flex flex-wrap gap-2">
                {['SessionStart', 'UserPromptSubmit', 'PreToolUse', 'PostToolUse', 'SubagentStop', 'Stop', 'Notification', 'Cron'].map(event => {
                  ...
                  return (
                    <div key={event} className="flex items-center gap-1 px-2 py-1 rounded bg-[rgba(255,255,255,0.03)]">
                      <span className="text-[10px] text-cloud-white">{event}</span>
                      {count > 0 && (
                        <span className="text-[9px] px-1 rounded-full bg-[rgba(255,107,107,0.1)] text-coral-energy">{count}</span>
                      )}
                    </div>
                  )
                })}
