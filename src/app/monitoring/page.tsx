            ...
            <div className="flex flex-wrap gap-2 mb-1">
              {checkTypes.map(type => (
                <div key={type} className="flex items-center gap-1">
                  ...
                  <span className="text-sm text-steel-gray uppercase">{type}</span>
              ...
              <div className="space-y-1.5">
                {MONITOR_CHECKS.map(check => (
                  <div key={check.id} className="flex items-center gap-3 py-1.5 px-3 rounded-lg bg-[rgba(255,255,255,0.05)]">
                    ...
                    />
                    <span className="text-base text-cloud-white w-36 shrink-0 truncate">{check.name}</span>
                    <span className="text-sm text-steel-gray w-20 shrink-0">{check.target}</span>
                    <span className="text-sm uppercase px-1.5 py-0.5 rounded shrink-0"
                          style={{ backgroundColor: `${CHECK_TYPE_COLORS[check.type]}15`, color: CHECK_TYPE_COLORS[check.type] }}>
                      {check.type}
                    </span>
                    {check.value && (
                      <span className="text-sm text-steel-gray ml-auto">{check.value}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        {view === 'alerts' && (
          <div className="glass-card p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-heading font-semibold text-cloud-white">Alert Rules</h3>
              <span className="text-sm text-steel-gray ml-auto">ntfy push notifications</span>
            </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h4 className="text-base font-heading font-semibold text-cloud-white">Local</h4>
                </div>
                <p className="text-sm text-steel-gray">/mnt/shared/backups/ — NFS sur r2d2-store</p>
                <p className="text-sm text-steel-gray">Retention: 14 jours</p>
              </div>
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-1">
                  <Shield className="w-4 h-4 text-neon-purple" />
                  <h4 className="text-base font-heading font-semibold text-cloud-white">Offsite</h4>
                </div>
                <p className="text-sm text-steel-gray">Google Drive via rclone</p>
                <p className="text-sm text-steel-gray">14.7 Go libres, sync quotidien</p>
              </div>
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-1">
                  <CheckCircle className="w-4 h-4 text-success-green" />
                  <h4 className="text-base font-heading font-semibold text-cloud-white">Restore Test</h4>
                </div>
                <p className="text-sm text-steel-gray">Mensuel, 4 backups testes</p>
                <p className="text-sm text-steel-gray">Containers ephemeres, 4/4 OK</p>
              </div>
            </div>
            ...
            <div className="glass-card p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-heading font-semibold text-cloud-white">Backup Jobs</h3>
              </div>
              <div className="space-y-2">
                {BACKUP_JOBS.map(job => (
                  <div key={job.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-[rgba(255,255,255,0.05)]">
                    ...
                    <span className="text-sm text-steel-gray flex-1 truncate">{job.target}</span>
                    <span className="text-sm text-steel-gray w-16 text-right">{job.retention}</span>
                    {job.lastSize && (
                      <span className="text-sm text-electric-cyan w-12 text-right">{job.lastSize}</span>
                    )}
                  </div>
                ))}
              </div>
