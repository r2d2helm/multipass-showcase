...
import { Shield, Radio, Layers, AlertTriangle, CheckCircle, XCircle, Eye, Wrench, Database, ChevronDown, ChevronUp } from 'lucide-react'
import {
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.06)]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm px-2 py-0.5 rounded-full bg-[rgba(255,107,107,0.1)] text-coral-energy">
            AdminSystem v2.5.0
          </span>
        </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-all shrink-0 ${
                tab === t.id
                  ? 'bg-[rgba(255,107,107,0.15)] text-coral-energy'
          ...
          <>
            ...
            <div className="glass-card p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-3 rounded-lg bg-[rgba(255,191,36,0.06)] border border-[rgba(255,191,36,0.15)]">
                ...
                <div className="p-3 rounded-lg bg-[rgba(0,212,255,0.06)] border border-[rgba(0,212,255,0.15)]">
              ...
              return (
                <div key={source} className="glass-card p-4">
                  ...
                  <div className="space-y-1.5">
                    {probes.map(probe => (
                      <div key={probe.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-[rgba(255,255,255,0.04)]">
                        ...
                        <span className="text-sm px-1.5 py-0.5 rounded bg-[rgba(0,212,255,0.1)] text-electric-cyan shrink-0">{probe.method}</span>
            ...
            {RECOVERY_LAYERS.map((layer, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 rounded-lg bg-[rgba(255,107,107,0.1)] flex items-center justify-center text-coral-energy text-base font-bold shrink-0">
                    L{i + 1}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-heading font-semibold text-cloud-white">{layer.name}</span>
                      <span className={`text-sm px-1.5 py-0.5 rounded ${layer.automated ? 'bg-[rgba(74,222,128,0.1)] text-success-green' : 'bg-[rgba(251,191,36,0.1)] text-amber-warm'}`}>
                        {layer.automated ? 'Auto' : 'Manuel'}
                      </span>
                      <span className="text-sm px-1.5 py-0.5 rounded bg-[rgba(0,212,255,0.1)] text-electric-cyan ml-auto">
                        RTO {layer.rto}
                      </span>
              ...
              return (
                <div key={cat} className="glass-card p-4">
                  ...
                  <div className="space-y-2">
                    {assets.map(asset => (
                      <div key={asset.id} className="p-3 rounded-lg bg-[rgba(255,255,255,0.04)]">
            ...
            {INCIDENT_SCENARIOS.map(incident => (
              ...
              >
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="text-sm px-1.5 py-0.5 rounded bg-[rgba(0,212,255,0.1)] text-electric-cyan">
                    RTO {incident.rto}
                  </span>
                  {incident.tested && (
                ...
                {expandedIncident === incident.id && (
                  <div className="mt-3 pt-3 border-t border-[rgba(255,255,255,0.08)] ml-5 space-y-2 animate-in fade-in duration-200">
                    <p className="text-sm text-steel-gray">
