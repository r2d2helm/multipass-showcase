      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        {vm && (
          <div className="glass-card p-4 animate-in fade-in duration-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-lg font-heading font-semibold text-cloud-white">{vm.name}</h3>
              <span className="text-sm text-steel-gray ml-auto">VMID {vm.vmid} — {vm.ip}</span>
            </div>
            <p className="text-base text-steel-gray mb-3">{vm.description}</p>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <h4 className="text-sm text-steel-gray uppercase tracking-wide mb-2">Services</h4>
                <div className="space-y-1">
                  {vm.services.map(s => (
                    <div key={s} className="flex items-center gap-2 text-base text-cloud-white">
              ...
              <div>
                <h4 className="text-sm text-steel-gray uppercase tracking-wide mb-2">Network Connections</h4>
                <div className="space-y-1">
                  {NETWORK_LINKS
        ...
        <div className="glass-card p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-heading font-semibold text-cloud-white">Network Topology</h3>
          </div>
            ...
            {NETWORK_LINKS.filter(l => l.protocol !== 'VM').map(link => {
              ...
              return (
                <div key={link.id} className="flex items-center gap-2 py-1.5 px-3 rounded-lg bg-[rgba(255,255,255,0.05)]">
                  ...
                  <span className="text-base text-cloud-white font-medium">{tgt?.name || link.target}</span>
                </div>
              )
            })}
          </div>
